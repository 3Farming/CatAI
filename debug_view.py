import atexit, json, math, os, struct, subprocess, sys, time, cv2
...
class DebugViewPublisher:
    ...
    def publish(self, frame, debug_data=None):
        ...
        try:
            ...
            if frame.base is self.frame_array or frame.flags["C_CONTIGUOUS"]:
...
def run_viewer_worker(
    ...
    try:
        while True:
            ...
            if frames_shown % 15 == 0 and time.perf_counter() - worker_started_at > 1.0:
                try:
