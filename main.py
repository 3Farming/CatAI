import inspect, os, sys, socket, webbrowser, time, threading
from utils import AsyncFrameReader

cap = AsyncFrameReader(0).start()

# ====== WATCHDOG start ======
from watchdog import GameWatchdog

self.watchdog = GameWatchdog(self.window_controller)

player_box = data.get("player")[0] if data.get("player") else None

if self.watchdog.process_anti_stuck(player_box):
    print("[Watchdog] Player is stuck! Executing micro-dash or changing direction.")

    self.window_controller.move(-100, -100) 

if movement is not None:
    smooth_movement = self.watchdog.apply_low_pass_filter(movement[0], movement[1])
    self.player.do_movement(smooth_movement)
...
def apply_play_order(queue_data):
    ...
    play_order = str(general_cfg.get("play_order", "in_order")).strip().lower()
...
def pyla_main(discord_bot, queue_data, stop_event=None, runtime_control=None):
    class Main:
        def __init__(self):
            ...
            gc.set_threshold(40000, 10, 10)
        def handle_detected_state(self, new_state):
            ...
            elif old_state == "match" and new_state != "match":
                gc.enable()
                gc.collect() 
            general_cfg = load_toml_as_dict("cfg/general_config.toml")
            bot_cfg = load_toml_as_dict("cfg/bot_config.toml")
            webhook_cfg = load_toml_as_dict("cfg/webhook_config.toml")
            thresholds_cfg = load_toml_as_dict("cfg/time_tresholds.toml")

            current_playstyle = bot_cfg.get("current_playstyle", "default_up.pyla")
            try:
                self.max_ips = int(general_cfg['max_ips'])
            except ValueError:
                self.max_ips = None
            ...
            self.run_for_minutes = int(general_cfg['run_for_minutes'])
            self.webhook_ping_every_minutes = webhook_cfg['ping_every_x_minutes']
            ...
            self.window_controller.get_latest_frame()
            ...
            self.check_if_brawl_stars_crashed_timer = thresholds_cfg["check_if_brawl_stars_crashed"]
            self.ping_when_stuck = webhook_cfg["ping_when_stuck"]
        ...
        def restart_brawl_stars(self):
            ...
            if not self.window_controller.is_brawl_stars_running():
                if self.ping_when_stuck:
                    ...
                    notify_user("bot_is_stuck", frame, self.Stage_manager)
        ...
        def sleep_interruptible(self, duration, allow_pause=True, poll_interval=0.05):
        ...
        def wait_while_paused(self):
            ...
            while self.should_pause() and not self.should_stop():
                ...
                if state is None:
                    if self.sleep_interruptible(0.1, allow_pause=False) == "stop":
                        return
                    continue
                if self.sleep_interruptible(0.2, allow_pause=False) == "stop":
                    return
        ...
        def manage_time_tasks(self, frame):
            ...
            if self.webhook_ping_every_minutes and current_time - self.time_since_last_webhook_ping >= self.webhook_ping_every_minutes * 60:
                ...
                notify_user("regular_minutes_ping", frame, self.Stage_manager)
        ...
        def main(self):
            ...
            while True:
                ...
                if frame is None or last_ft <= last_processed_frame_time:
                    time.sleep(0.002)
                    continue
                ...
                if not self.picked_first_brawler and self.get_latest_state() == "lobby":
                    if self.Stage_manager.brawlers_pick_data[0]['automatically_pick']:
                        ...
                        while select_brawler in ["failed", "error"]:
                            ...
                            if self.ping_when_stuck:
                                notify_user("bot_failed_brawler_selection", frame, self.Stage_manager)
