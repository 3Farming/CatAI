import hashlib, html, io, math, os, random, ssl, threading, time, ctypes, json, requests, toml, cv2, traceback
...
def load_toml_as_dict(file_path, cache=True):
    ...
    if cache and str(full_path) in cached_toml:
...
def get_brawler_info(brawler_name):
    ...
    try:
        response = requests.post(url, json={'brawler_name': brawler_name}, timeout=10)
        if response.status_code == 200:
            return response.json().get('info', [])
    except Exception as e:
        print(f"Error fetching info for '{brawler_name}': {e}")
    return None
...
def save_brawler_icon(brawler_name):
    ...
    try:
        response = requests.get(brawlers_url, timeout=10)
        if response.status_code != 200:
            return
        ...
        print(f"Failed to handle icon for {brawler_name}: {e}")
...
def notify_user(message_type, screenshot, stage_manager) -> None:
    ...
    user_id = webhook_cfg.get("discord_id", "").strip()
    webhook_url = webhook_cfg.get("webhook_url", "").strip()
    telegram_token = webhook_cfg.get("telegram_token", "").strip()
    telegram_chat_id = webhook_cfg.get("telegram_chat_id", "").strip()
    ...
    if has_telegram:
        try:
            ...
            if image_buffer is not None:
                ...
                requests.post(url, data={"chat_id": telegram_chat_id, "caption": safe_text, "parse_mode": "HTML"}, files={"photo": ("screenshot.png", image_buffer, "image/png")}, timeout=15)
            else:
                url = f"https://api.telegram.org/bot{telegram_token}/sendMessage"
                requests.post(url, data={"chat_id": telegram_chat_id, "text": safe_text, "parse_mode": "HTML"}, timeout=15)
        except Exception as e:
...
def get_latest_wall_model_file():
    ...
    try:
        ...
        print(f"Failed to download the latest wall model: {e}")
...
def load_pyla_script(filename):
    ...
    try:
        ...
        print(f"An error occurred while loading the .pyla script {filename}: {e}")
        return "", ""
